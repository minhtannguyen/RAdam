...
def main():
    ...
    #     optimizer = optim.Adam(model.parameters(), lr=args.lr, betas=(args.beta1, args.beta2), weight_decay=args.weight_decay)
        ...
        iter_count = 1
        optimizer = SRNAdam(model.parameters(), lr=args.lr, iter_count=iter_count, restarting_iter=args.restart_schedule[0]) 
    ...
    for epoch in range(start_epoch, args.epochs):
        if args.optimizer.lower() == 'srsgd':
            ...
            if epoch in args.schedule:
                optimizer = SRNAdam(model.parameters(), lr=args.lr * (args.gamma**schedule_index), iter_count=iter_count, restarting_iter=args.restart_schedule[schedule_index])
                schedule_index += 1
