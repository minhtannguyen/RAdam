...
def train(train_loader, model, criterion, optimizer, epoch, use_cuda, logger):
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        # plot progress
        bar.suffix  = '(Epoch {epoch}, LR {lr} {batch}/{size}) Data: {data:.3f}s/{data_global:.3f}s | Batch: {bt:.3f}s/{bt_global:.3f}s | Total: {total:} | ETA: {eta:} | Loss: {loss:.4f} | top1: {top1: .4f} | top5: {top5: .4f}'.format(
                    epoch=epoch,
