    ...
    for epoch in range(start_epoch, args.epochs):
        if args.optimizer.lower() == 'srsgd':
            if epoch == args.schedule[0]:
                optimizer = SGD_Adaptive(model.parameters(), lr=args.lr * args.gamma, weight_decay=args.weight_decay, iter_count=iter_count, restarting_iter=args.restart_schedule[1])
            elif epoch == args.schedule[1]:
                optimizer = SGD_Adaptive(model.parameters(), lr=args.lr * (args.gamma**2), weight_decay=args.weight_decay, iter_count=iter_count, restarting_iter=args.restart_schedule[2])
            elif epoch == args.schedule[2]:
                optimizer = SGD_Adaptive(model.parameters(), lr=args.lr * (args.gamma**3), weight_decay=args.weight_decay, iter_count=iter_count, restarting_iter=args.restart_schedule[3])
            elif epoch == args.schedule[3]:
                optimizer = SGD_Adaptive(model.parameters(), lr=args.lr * (args.gamma**4), weight_decay=args.weight_decay, iter_count=iter_count, restarting_iter=args.restart_schedule[4])   
        elif args.optimizer.lower() == 'sradam':
            if epoch == 80:
