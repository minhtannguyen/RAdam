...
def get_loaders_eval(dataset, args):
    ...
    if dataset == 'cifar10':
        ...
        num_classes = 200
        train_transform, valid_transform = _data_transforms_tinyimagenet(args)
        train_loc = args.data + '/tiny-imagenet-200/train'
        valid_loc = args.data + '/tiny-imagenet-200/val'
        train_data = dset.ImageFolder(
            root=train_loc, transform=train_transform)
        valid_data = dset.ImageFolder(
            root=valid_loc, transform=valid_transform)
    elif dataset == 'tim20':
        num_classes = 20
        train_transform, valid_transform = _data_transforms_tinyimagenet(args)
        train_loc = args.data + '/tiny-imagenet-20/train'
        valid_loc = args.data + '/tiny-imagenet-20/val'
        train_data = dset.ImageFolder(
            root=train_loc, transform=train_transform)
        valid_data = dset.ImageFolder(
            root=valid_loc, transform=valid_transform)
...
def get_loaders_search(args):
    ...
    if args.dataset == 'cifar10':
        ...
        train_data = dset.CIFAR100(
            root=args.data, train=True, download=True, transform=train_transform)
        # # Get only 10 classes from CIFAR-100
        # print(train_data.train_data.shape)
        # chosen_classes = torch.randperm(100)
        # chosen_classes = list(chosen_classes.numpy())
        # chosen_classes = chosen_classes[:10]
        # print('Chosen class labels:', chosen_classes)
        # indices, labels = [], []
        # for item in train_data.train_labels:
        #     if item in chosen_classes:
        #         indices.append(True)
        #         labels.append(item)
        #     else:
        #         indices.append(False)
        # train_data.train_data = train_data.train_data[indices, ...]
        # train_data.train_labels = labels
        # print(train_data.train_data.shape)
        # print(len(train_data.train_labels))
        # print('********WARNING: ONLY CHOOSING 10 CLASSES**********')
        # input()
    elif args.dataset == 'tim200':
        num_classes = 200
        train_transform, _ = _data_transforms_tinyimagenet(args)
        train_loc = args.data + '/tiny-imagenet-200/train'
        train_data = dset.ImageFolder(
            root=train_loc, transform=train_transform)
    elif args.dataset == 'tim20':
        num_classes = 20
        train_transform, _ = _data_transforms_tinyimagenet(args)
        train_loc = args.data + '/tiny-imagenet-20/train'
        train_data = dset.ImageFolder(
