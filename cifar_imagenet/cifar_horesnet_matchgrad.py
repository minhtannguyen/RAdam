...
parser.add_argument('--test-batch', default=100, type=int, metavar='N',
                    help='test batchsize')
parser.add_argument('--optimizer', default='sgd', type=str, choices=['adamw', 'radam', 'sgd', 'srsgd'])
...
def train(trainloader, model, criterion, optimizer, epoch, use_cuda, logger):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        # compute output
        outputs, x_grad_est, x_1 = model(inputs)
        loss = criterion(outputs, targets)
        
        ####################################################################
        
        x_grad = torch.autograd.grad(loss, x_1, create_graph=True, retain_graph=True)[0]
        reg_grad = l2loss(x_grad_est, x_grad)
        
        loss = loss + 0.1 * reg_grad
